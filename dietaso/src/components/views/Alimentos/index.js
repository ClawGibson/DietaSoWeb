...
const Alimentos = () => {
    ...
    */
    ...
    const borrar = (value) => {
        try {
            const borrado = data2.opcionesPreparacion.filter((opcion) => opcion.toLowerCase().trim() !== value.toLowerCase().trim());
            console.log(borrado)
            setData({
                ...data2,
    ...
    return (
        <div class="container" >
            ...
            <IconsComponent img={data2} nutricional={handleNutricional} ambiental={handleAmbiental} economia={handleEconomia} sociedad={handleCulturaSociedad} />
            <PropertiesComponent borrar={borrar} item={data2} getData={getData} setShowAlimento={setShowAlimento} handleOk={handleOk} showModal={showModal} isModalVisible={isModalVisible} handleCancel={handleCancel} />
        </div>
    );
};
